...
class ClutchSub(Node):
    def __init__(self):
        super().__init__("Alice_Subscriber")
        self.clutch_subscriber = self.create_subscription(
            Bool, "/footswitch1", self.clutch_sub_callback, 10) 
        
        self.io_state_subsriber = self.create_subscription(
            IOStates, "/alice/io_and_status_controller/io_states", self.state_sub_callback, 10) 
    ...
    def clutch_sub_callback (self, msg: Bool): 
        ...
        if clutch_data and not self.prev_clutch_state:
            ...
            if self.gripper_closed: 
                self.get_logger().info("Open Alice's Gripper")
                ...
                self.get_logger().info("Close Alice's Gripper")
    ...
    def state_sub_callback(self, msg: IOStates):
        if len(msg.digital_out_states) > 2:
            ...
            if self.gripper_closed is None:
                ...
                self.get_logger().info(f"Alice's Initial gripper state set to: {'Closed' if self.gripper_closed else 'Open'}")


        
    def call_setio_service(self, fun, pin, state): 
        client = self.create_client(SetIO, "/alice/io_and_status_controller/set_io")
